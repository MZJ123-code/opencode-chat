...
bun run dev              # 同时启动后端 (Express :3000) + 前端 (Vite HMR @ :5173)
cd client && bun run build && cd .. # 构建前端 → dist/
bun start                # 生产启动
bun run dev:external     # 连接已有外部 OpenCode 进程
```
...
| 命令 | 说明 |
|------|------|
| `bun run dev` | 开发模式，同时启动后端（--watch 热重启）+ 前端 Vite HMR |
| `bun start` | 生产启动 |
